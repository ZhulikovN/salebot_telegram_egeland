...
from app.services.telegram_relay_client import (
    CAPTION_LIMIT,
    TelegramRelayClient,
    TelegramSendError,
    supports_media_type,
...
class ConversationManager:
    ...
    async def handle_amojo_message(
        ...
        # Вложение отправляем напрямую в Telegram через relay, если для бота есть токен.
        if media_url:
            ...
            if await self._send_media_via_relay(
                conversation=conversation,
    ...
    async def _send_media_via_relay(
        self,
        conversation: Conversation,
        message_type: str,
        media_url: str,
        ...
        """
        Отправить вложение клиенту напрямую через Telegram Bot API (через relay).

        Наш сервер не может напрямую достучаться до api.telegram.org, а Salebot
        передаёт Telegram только ссылку на файл, которую его серверы не могут
        скачать с нашего хостинга — клиент получает текстовую ссылку вместо
        картинки. Поэтому для ботов с настроенным токеном файл грузится байтами
        через отдельный relay-сервис (см. relay/), который может достучаться
        до Telegram напрямую.
        ...
        if not token or not settings.TELEGRAM_RELAY_URL or not supports_media_type(message_type):
            return False
        ...
        try:
            ...
            logger.error(
                "Relay send failed, falling back to Salebot: bot=%s, type=%s, %s",
                conversation.bot_name,
                message_type,
                e,
            )
