"""Процессор для таблиц Неоплаты (12 таблиц, обрабатываются раз в месяц)."""
...
class NeoplatyProcessor(BaseSheetProcessor):
    """Универсальный процессор для таблиц Неоплаты."""
    ...
    def __init__(self, bot_name: str, *args, **kwargs):
        ...
        Args:
            bot_name: Название бота для этой таблицы (например: "Неоплаты Физика")
        """
    ...
    def parse_row(self, row: Dict[str, Any]) -> Dict[str, Any]:
        """
        Парсинг строки таблицы Неоплаты.
        ...
        # Опциональные поля
        course_name = str(row.get("Название курса", "")).strip() or None
        subject = str(row.get("Предмет", "")).strip() or None
    ...
    async def process_deal(self, data: Dict[str, Any]) -> None:
        ...
        2. Если есть tg_id - поиск клиента в Salebot (если нет - ошибка "Диалога нет")
        ...
        self.logger.info(
            "Processing deal: tg_id=%s, phone=%s, student=%s",
        ...
        # 3. Создание чата и перенос истории (только если есть tg_id и salebot_client_id)
    ...
    # get_status_column и get_error_column используют автопоиск из базового класса
